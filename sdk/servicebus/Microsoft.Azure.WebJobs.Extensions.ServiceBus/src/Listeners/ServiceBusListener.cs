...
#pragma warning restore AZC0102 // Do not use GetAwaiter().GetResult().
        ...
        private async Task RunBatchReceiveLoopAsync(CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    if (_concurrencyManager.Enabled)
                    {
                        // Dynamic concurrency is enabled so consult ConcurrencyManager to see if there are any available invocations.
                        // This batch receive loop only executes one function at a time, so we only need to check here if we have 1 or
                        // more available invocations. Because we only invoke 1 function at a time below (serially), we'll never be executing
                        // functions concurrently. So the only purpose this DC check serves currently is to skip invocations if throttles
                        // are enabled (i.e. AvailableInvocationCount is 0).
                        var concurrencyStatus = _concurrencyManager.GetStatus(_functionId);
                        if (concurrencyStatus.AvailableInvocationCount == 0)
                        {
                            await Task.Delay(concurrencyStatus.NextStatusDelay, cancellationToken).ConfigureAwait(false);
                            continue;
                        }
                    }
        ...
        private class DynamicServiceBusProcessorStrategy : ServiceBusProcessorStrategy
        {
            ...
            public override ReceiverStatus GetReceiverStatus()
            {
                ...
                // to the current concurrency level. So below, it suffices to check whether AvailableInvocationCount
                // is greater than zero. If throttles are currently enabled, 0 will be returned and the receive loop
                // will continue without receiving a message.
                var concurrencyStatus = _concurrencyManager.GetStatus(_functionId);

                return new ReceiverStatus
                {
                    CanReceive = concurrencyStatus.AvailableInvocationCount > 0
                };
            }
